...
#######################################################################################
webpage::makeForm_WebpageHeaderAndyStyle( $cgi, $formpath, "ABUS", "Read ALL ABUS Nodes" );
print "<br>";
...
if (@readoption) {
    ...
    for ( my $count = 0 ; $count < scalar(@ABUSRegisterHashArray) ; $count++ ) {
        ($ABUSPhysicalReading, $DMMMeasurement) = ABUS::BitBangABUSNodeRead( \@CurrentRegStateHashArray, $ABUSRegisterHashArray[$count], $DMM );
        push @$ABUSTable,
          $cgi->td(
            [
