...
print "<H2>Read Temperature Sensors</H2>\n";
...
print "There are about 9 ABUS Nodes to be measured<br>";
print
...
my $ReadButtonText = "Read Temperatures";
...
if (@readoption) {
    ...
    my $ABUSTable = [ $cgi->th( [ "Node", "TURN ME OFF Value", "Measured Value" ] ) ];
    ...
    for ( my $count = 0 ; $count < scalar(@ABUSRegisterHashArray) ; $count++ ) {
        ...
        if ( $CurrentABUSNodeNameSplit[0] eq "TempABUS" ) {
            $ABUSPhysicalReading =
